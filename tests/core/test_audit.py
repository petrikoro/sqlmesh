...
def test_macro(model: Model):
    expected_query = """SELECT * FROM "db"."test_model" AS "test_model" WHERE "a" IS NULL"""
...
def test_not_null_audit(model: Model):
    ...
    assert (
        ...
        == """SELECT * FROM "db"."test_model" AS "test_model" WHERE ("a" IS NULL OR "b" IS NULL) AND TRUE"""
    )
...
def test_not_null_audit_default_catalog(model_default_catalog: Model):
    ...
    assert (
        rendered_query_a.sql()
        == """SELECT * FROM "test_catalog"."db"."test_model" AS "test_model" WHERE "a" IS NULL AND TRUE"""
    )
    ...
    assert (
        rendered_query_a_and_b.sql()
        == """SELECT * FROM "test_catalog"."db"."test_model" AS "test_model" WHERE ("a" IS NULL OR "b" IS NULL) AND TRUE"""
    )
...
def test_unique_values_audit(model: Model):
    ...
    assert (
        rendered_query_a.sql()
        == 'SELECT * FROM (SELECT ROW_NUMBER() OVER (PARTITION BY "a" ORDER BY "a") AS "rank_a" FROM "db"."test_model" AS "test_model" WHERE "b" IS NULL) AS "_0" WHERE "rank_a" > 1'
    )
    ...
    assert (
        rendered_query_a_and_b.sql()
        == 'SELECT * FROM (SELECT ROW_NUMBER() OVER (PARTITION BY "a" ORDER BY "a") AS "rank_a", ROW_NUMBER() OVER (PARTITION BY "b" ORDER BY "b") AS "rank_b" FROM "db"."test_model" AS "test_model" WHERE TRUE) AS "_0" WHERE "rank_a" > 1 OR "rank_b" > 1'
    )
...
def test_accepted_values_audit(model: Model):
    ...
    assert (
        rendered_query.sql()
        == """SELECT * FROM "db"."test_model" AS "test_model" WHERE NOT "a" IN ('value_a', 'value_b') AND TRUE"""
    )
...
def test_number_of_rows_audit(model: Model):
    ...
    assert (
        rendered_query.sql()
        == """SELECT COUNT(*) FROM (SELECT 1 FROM "db"."test_model" AS "test_model" WHERE TRUE LIMIT 0 + 1) AS "_0" HAVING COUNT(*) <= 0"""
    )
...
def test_forall_audit(model: Model):
    ...
    assert (
        rendered_query_a.sql()
        == """SELECT * FROM "db"."test_model" AS "test_model" WHERE NOT ("a" >= "b") AND TRUE"""
    )
    ...
    assert (
        rendered_query_a.sql()
        == """SELECT * FROM "db"."test_model" AS "test_model" WHERE (NOT ("a" >= "b") OR NOT ("c" + "d" - "e" < 1.0)) AND TRUE"""
    )
    ...
    assert (
        rendered_query_a.sql()
        == """SELECT * FROM "db"."test_model" AS "test_model" WHERE (NOT ("a" >= "b") OR NOT ("c" + "d" - "e" < 1.0)) AND "f" = 42"""
    )
...
def test_accepted_range_audit(model: Model):
    ...
    assert (
        rendered_query.sql()
        == 'SELECT * FROM "db"."test_model" AS "test_model" WHERE "a" < 0 AND TRUE'
    )
    rendered_query = model.render_audit_query(
    ...
    assert (
        rendered_query.sql()
        == 'SELECT * FROM "db"."test_model" AS "test_model" WHERE "a" >= 100 AND TRUE'
    )
    rendered_query = model.render_audit_query(
    ...
    assert (
        rendered_query.sql()
        == 'SELECT * FROM "db"."test_model" AS "test_model" WHERE ("a" < 100 OR "a" > 100) AND TRUE'
    )
...
def test_at_least_one_audit(model: Model):
    ...
    assert (
        rendered_query.sql()
        == 'SELECT 1 AS "1" FROM "db"."test_model" AS "test_model" WHERE TRUE GROUP BY 1 HAVING COUNT("a") = 0'
    )
...
def test_sequential_values_audit(model: Model):
    ...
    assert (
        rendered_query.sql()
        == '''WITH "windowed" AS (SELECT "a", LAG("a") OVER (ORDER BY "a") AS "prv" FROM "db"."test_model" AS "test_model" WHERE TRUE), "validation_errors" AS (SELECT * FROM "windowed" AS "windowed" WHERE NOT ("a" = "prv" + 1)) SELECT * FROM "validation_errors" AS "validation_errors"'''
    )
...
def test_pattern_audits(model: Model):
    ...
    assert (
        rendered_query.sql()
        == """SELECT * FROM "db"."test_model" AS "test_model" WHERE (NOT REGEXP_LIKE("a", \'^\\d.*\') AND NOT REGEXP_LIKE("a", \'.*!$\')) AND TRUE"""
    )
    ...
    assert (
        rendered_query.sql()
        == """SELECT * FROM "db"."test_model" AS "test_model" WHERE (REGEXP_LIKE("a", \'^\\d.*\') OR REGEXP_LIKE("a", \'.*!$\')) AND TRUE"""
    )
    ...
    assert (
        rendered_query.sql()
        == """SELECT * FROM "db"."test_model" AS "test_model" WHERE (NOT "a" LIKE \'jim%\' AND NOT "a" LIKE \'pam%\') AND TRUE"""
    )
    ...
    assert (
        rendered_query.sql()
        == """SELECT * FROM "db"."test_model" AS "test_model" WHERE ("a" LIKE \'jim%\' OR "a" LIKE \'pam%\') AND TRUE"""
    )
...
def test_string_length_between_audit(model: Model):
    ...
    assert (
        rendered_query.sql()
        == """SELECT * FROM "db"."test_model" AS "test_model" WHERE (LENGTH("x") < 1 OR LENGTH("x") > 5) AND TRUE"""
    )
...
def test_not_constant_audit(model: Model):
    ...
    assert (
        rendered_query.sql()
        == """SELECT 1 AS "1" FROM (SELECT COUNT(DISTINCT "x") AS "t_cardinality" FROM "db"."test_model" AS "test_model" WHERE "x" > 1) AS "r" WHERE "r"."t_cardinality" <= 1"""
    )
...
def test_condition_with_macro_var(model: Model):
    ...
    assert (
        ...
        == """SELECT * FROM "db"."test_model" AS "test_model" WHERE "x" IS NULL AND "dt" BETWEEN CAST('1970-01-01 00:00:00+00:00' AS TIMESTAMPTZ) AND CAST('1970-01-01 23:59:59.999999+00:00' AS TIMESTAMPTZ)"""
    )
