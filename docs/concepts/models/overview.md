...
:   Indicates that this model can be executed for partial (incomplete) data intervals.
    ...
    The `--ignore-cron` flag can also be used with `sqlmesh plan` to include partial intervals in the backfill range. This is useful when production processes partial intervals (via `sqlmesh run --ignore-cron`) and you want a development environment to cover the same time range: `sqlmesh plan dev --ignore-cron`.
